...
const goUp = (entryPoint, steps, array, intersect, intersectionsArray) => {
  let endPoint = [0, 0]
  for (let x = 1; x <= steps; x++) {
    if (intersect &&
      array[entryPoint[0] + x][entryPoint[1]] === 1) {
      array[entryPoint[0] + x][entryPoint[1]] = 2
      intersectionsArray.push([entryPoint[0] + x, entryPoint[1]])
     
    }
    else {
      array[entryPoint[0] + x][entryPoint[1]] = intersect ? 3 : 1
    }
    endPoint = [entryPoint[0] + x, entryPoint[1]]
  }
  return endPoint
}

const goDown = (entryPoint, steps, array, intersect, intersectionsArray) => {
  let endPoint = [0, 0]
  for (let x = 1; x <= steps; x++) {
    if (
      intersect &&
...
const processInstruction = (entryPoint, instruction, array, intersect, intersectionsArray) => {
  ...
  return commands[direction](entryPoint, steps, array, intersect, intersectionsArray)
}

const processWire = (initialEntryPoint, instructionString, array, intersect, intersectionsArray) => {
  const instructionArray = instructionString.split(',')

  console.log(instructionArray)

  let entryPoint = initialEntryPoint

  for (instruction of instructionArray) {
    entryPoint = processInstruction(entryPoint, instruction, array, intersect, intersectionsArray)
  }
}

const core = [~~(size / 2), ~~(size / 2)]

intersectionsArray = []

processWire(core, wireOne, grid, false, intersectionsArray)
processWire(core, wireTwo, grid, true, intersectionsArray)

distanceFromCoreArray = intersectionsArray.map(intersectionsPoint => {
  return Math.abs(core[0] - intersectionsPoint[0]) + Math.abs(core[1] - intersectionsPoint[1])
})

console.log(distanceFromCoreArray.reduce((acc, val) => val < acc ? val : acc,6000))
...
const goRight = (entryPoint, steps, array, intersect, intersectionsArray, currentSteps) => {
  let endPoint = [0, 0]
  for (let x = 1; x <= steps; x++) {
    if (intersect &&
      array[entryPoint[0]][entryPoint[1] + x][0] === 1) {

      intersectionsArray.push([entryPoint[0], entryPoint[1] + x, array[entryPoint[0]][entryPoint[1] + x][1],currentSteps + x])
    }
    else {
      array[entryPoint[0]][entryPoint[1] + x] = intersect ? 3 :
       array[entryPoint[0]][entryPoint[1] + x] ?
        array[entryPoint[0]][entryPoint[1] + x] : [1,currentSteps + x]
    }
    endPoint = [entryPoint[0], entryPoint[1] + x]
  }
  return endPoint
...
const goLeft = (entryPoint, steps, array, intersect, intersectionsArray, currentSteps) => {
  let endPoint = [0, 0]
  for (let x = 1; x <= steps; x++) {
    if (intersect &&
      array[entryPoint[0]][entryPoint[1] - x][0] === 1) {
...
const goUp = (entryPoint, steps, array, intersect, intersectionsArray,currentSteps) => {
  let endPoint = [0, 0]
  for (let x = 1; x <= steps; x++) {
    if (intersect &&
      array[entryPoint[0] + x][entryPoint[1]][0] === 1) {
       intersectionsArray.push([entryPoint[0] + x, entryPoint[1], array[entryPoint[0] + x][entryPoint[1] ][1], currentSteps + x])
     
    }
    else {
      array[entryPoint[0] +x][entryPoint[1]] = intersect ? 3 :
       array[entryPoint[0] +x][entryPoint[1]] ?
        array[entryPoint[0]+x][entryPoint[1]] : [1,currentSteps + x]
    }
    endPoint = [entryPoint[0] + x, entryPoint[1]]
  }
  return endPoint
...
const goDown = (entryPoint, steps, array, intersect, intersectionsArray, currentSteps) => {
  let endPoint = [0, 0]
  for (let x = 1; x <= steps; x++) {
    if (
      intersect &&
...
const processInstruction = (entryPoint, instruction, array, intersect, intersectionsArray, currentSteps) => {
  const direction = instruction.substring(0, 1)
  const steps = Number(instruction.substring(1))
  ...
  return [commands[direction](entryPoint, steps, array, intersect, intersectionsArray,currentSteps), currentSteps+steps]
}
...
const processWire = (initialEntryPoint, instructionString, array, intersect, intersectionsArray) => {
  ...
  for (instruction of instructionArray) {
    processedInstruction = processInstruction(entryPoint, instruction, array, intersect, intersectionsArray, stepCounter)

    entryPoint = processedInstruction[0]
    stepCounter = processedInstruction[1]
  }
}
...
const stepsArray = intersectionsArray.map(data => data[2] + data[3])

console.log(stepsArray.reduce((acc, val)=> val < acc ? val :acc))
